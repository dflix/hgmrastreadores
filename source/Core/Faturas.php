...
//            $vercarteira->ExeRead("app_carterias", "WHERE id = :id", "id={$data["carteira_id"]}");
...
//var_dump($Dados , $data , $DadosUp);
    ...
    public function registraFatura() {
        $data = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRIPPED);
        ...
        if (!empty($data["modo"])) {
            ...
            //verifica se existe uma assinatura valida
            $vercarteira = new \Source\Models\Read();
            $vercarteira->ExeRead("app_carterias", "WHERE wallet = :id", "id={$data["carteira_id"]}");
            $vercarteira->getResult();
            $view = $vercarteira->getResult()[0]["wallet"];
            if($view = $vercarteira->getResult()[0]["wallet"] != "Carteira Free" && $_SESSION["nivel"] == "0"){
                echo "<div class=\"alert alert-danger\" role=\"alert\">
...
//var_dump($Dados , $data , $DadosUp);
    ...
    public function proxPagto() {
        ...
        if (!empty($_SESSION["carteira"])) {
            ...
            $read->ExeRead("app_faturas", "WHERE carteira_id = :c AND status = :s  AND modo = :m AND vencimento_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH)",
                    "m=saida&s=unpaid&c=Empresa");
            $read->getResult();

            return $read->getResult();
        }
        $read = new \Source\Models\Read();
        $read->ExeRead("app_faturas", "WHERE status = :s  AND modo = :m AND vencimento_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH)", "m=saida&s=unpaid");
        $read->getResult();
    ...
    public function proxPagtoFixa() {
        ...
        if (!empty($_SESSION["carteira"] == "geral")) {
            ...
            $read = new \Source\Models\Read();
            $read->ExeRead("app_faturas", "WHERE status = :s AND js_fixa = :j AND modo = :m AND repetir_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH)", "m=saida&s=paid&j=0");
            $read->getResult();

            return $read->getResult();
        } else {
            $read = new \Source\Models\Read();
            $read->ExeRead("app_faturas", "WHERE  js_fixa = :j AND status = :s AND carteira_id = :c AND modo = :m AND repetir_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH)", "s=paid&c={$_SESSION['carteira']}&m=saida&j=0");
            $read->getResult();
    ...
    public function proxReceita() {
        ...
        if (!empty($_SESSION["carteira"])) {
            $read = new \Source\Models\Read();
            $read->ExeRead("app_faturas", "WHERE carteira_id = :c AND status = :s  AND modo = :m  AND vencimento_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH)",
                    "m=entrada&s=unpaid&c={$_SESSION["carteira"]}");
            return $read->getResult();
        }

        $read = new \Source\Models\Read();
        $read->ExeRead("app_faturas", "WHERE status = :s  AND modo = :m  AND vencimento_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH)", "m=entrada&s=unpaid");
        return $read->getResult();
    }
    ...
    public function proxReceitaFixa() {
        ...
        $read = new \Source\Models\Read();
        $read->ExeRead("app_faturas", "WHERE modo = :m  AND repetir_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH)", "m=entrada");
        return $read->getResult();
    }
    ...
    public function Fixas() {
        ...
        $read = new \Source\Models\Read();
        $read->ExeRead("app_faturas", "WHERE  js_fixa = :j AND tipo = :t AND repetir_em BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 1 MONTH) ",
                "t=Fixa&j=0");
        $read->getResult();
