...
<section class="buscar"> 
    ...
    <form action="index.php?p=eacasp" name="buscar" method="POST" enctype="multipart-form/data"> 
        
        <label class="seletor"> 
            
            <input type="radio" name="filtro" value="associado"/> Nome<br />
            <input type="radio" name="filtro" value="contrato"/> Contrato<br />
            <input type="radio" name="filtro" value="placa1"/> Placa<br />
...
<table width="100%" border="0">
  ...
  <tbody>
      ...
      foreach ($exibe->getResult() as $value) {
          ...
          if(!empty($atendimento->getResult()[0]['data'])):
           ...
           $dataatendimento = date("d/m/Y", strtotime($atendimento->getResult()[0]['data'])); 
               ...
               $historia = "nada consta";
           $dataatendimento = "sem atendimentos registrados";
          endif;
          ...
          if(isset($pagamento->getResult()[0]['data'])):
           $datapagamento = $pagamento->getResult()[0]['data']; 
              ...
              $datapagamento = "não ha registros de pagamento";
          endif;
          ...
          echo"      <tr style=\" font-family:Tahoma, Geneva, sans-serif; font-size:12px; color:#333;\">
           <td>{$dataatual}</td>
    <td><button type=\"button\" class=\"btn btn-xs btn-primary\" data-toggle=\"modal\" data-target=\"#myModal{$value['contrato']}\">TODOS<br>
      DADOS</button> </td>
    ...
    <td>
        ...
        <p> <a href=\"index.php?p=tacasp&id={$value['id_venda']}\">Transferência</a> </p>
        </br>
        <hr>
        </br>
        <p> <a href=\"transferencia-acasp.php?id={$value['id_venda']}\" target=\"blank\">Imprimir</a> </p>
         </br>
    ...
    <td><a href=\"index.php?p=aacasp&id={$value['id_venda']}\"><img src=\"img/editar.png\" /></a></td>
 <td><a href=\"index.php?p=eacasp&del={$value['id_venda']}\"><img src=\"img/deletar.png\" /></a></td>
  
                   
          
      </tr>
      
    								<div class=\"modal fade\" id=\"myModal{$value['contrato']}\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"myModalLabel\">
										...
										<div class=\"modal-content\">
											...
											<div class=\"modal-body\">
                                                                                        <p><b>Adesão: </b> {$dataatual} </p>
                                                                                                <p><b>Contrato:</b>{$value['contrato']}  <b> Nome: </b> {$puxapag} </p>
                                                                                                <p><b>Placa: </b> {$value['placa1']}  <b> Veiculo Modelo: </b> {$value['marca_modelo1']}  <b> Veiculo Marca:</b>{$value['marca_modelo1']}</p>
                                                                                                <p><b>Cor:</b>{$value['cor1']}  <b> Ano: </b> {$value['ano1']}</p>
...
<input type=\"hidden\" value=\"{$value['contrato']}\" name=\"\" />
</label>
</br>
