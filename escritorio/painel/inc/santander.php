...
//ler o conteudo do arquivo
    ...
    while (!feof($arquivo)) {
        ...
        echo "Letra da placa " . $letraplaca = substr($letra, 0, 3);
        
        echo "</br>";
        $texto = $string_letras;
        $numeros = preg_replace("/[^a0-z9]/", "", $texto);
        $numerosplaca = substr($numeros, 0, 4);
        
        echo "Numeto da placa é" . $numerosplaca ;
...
//            
//
//            echo "<p class=\"deletar\"><b>Erro ao atualizar baixa </b>- cadastrar manual - busca no banco por <b><a href=\"index.php?p=santandermanual&q={$nomecli}\" target=\"_blank\"> {$nomecli} </a></b></p></br>";
//
//            endif;
//        endif;
//
...
//echo "</br>";
//echo "O dia de pagamento é {$datapgto}";
//
//        $Dados = [
//            "data" => $datapgto,
//            "entrada" => $soma
...
//
//$vermov = new Read();
//$vermov->ExeRead("movsantander", "WHERE data = :p", "p={$datapgto}");
//$vermov->getResult();
//
//if ($vermov->getResult()):
//
////atualiza aqui
//    $up = new Update();
//    $up->ExeUpdate("movsantander", $Dados, "WHERE data = :p", "p={$datapgto}");
//    $up->getResult();
//    
//    if($up->getResult()):
//        
//        echo "<div class=\"alert alert-success\" role=\"alert\">Atualização do dia {$datapgto}  realizada com sucesso total R$ " . number_format($soma / 100, 2, ",", ".") . " </div>";
//        
//        else:
//            
//            echo "<div class=\"alert alert-danger\" role=\"alert\">Ops, deu merda, chame o Disbiriflix</div>";
//        
//    endif;
//
//else:
...
//    // cadastro novo
//    $mov = new Create();
//    $mov->ExeCreate("movsantander", $Dados);
//    $mov->getResult();
//    if ($mov->getResult()):
//         echo "<div class=\"alert alert-success\" role=\"alert\">Cadastro do dia {$data} realizada com sucesso, totalde R$ {$soma}</div>";
//         else:
//          echo "<div class=\"alert alert-danger\" role=\"alert\">Ops, deu merda, chame o Disbiriflix</div>";   
//    endif;
//
