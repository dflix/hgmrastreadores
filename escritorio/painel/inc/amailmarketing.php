<div class="col-md-12"> 
    <h3>EDITAR PROPOSTAS </h3>
     <?php
        $form = filter_input_array(INPUT_POST, FILTER_DEFAULT);
        
        if($form['sendpost']):
            unset($form['sendpost']);
            
        $up = new Update();
        $up->ExeUpdate("ws_marketing", $form, "WHERE id_proposta = :p", "p={$_GET['id']}");
        $up->getResult();
        if($up->getResult()):
            echo "<div class=\"alert alert-success\" role=\"alert\"><B>PROPOSTA ATULIZADA</B> com sucesso</div>";
            else:
            echo "<div class=\"alert alert-danger\" role=\"alert\">ERRO , error , <b>FUDEU</b></div>"; 
        endif;
            
       //var_dump($form);
            endif;
        

        $ver = new Read();
        $ver->ExeRead("ws_marketing", "WHERE id_proposta = :p", "p={$_GET['id']}");
        $ver->getResult();
      
        ?>
    
    
    <form name="PostForm" action="" method="post" enctype="multipart/form-data" >

            <div class="form-group"> 
                <p class="fonteform"> NOME AUTO RESPONDER </p>
                <input type="text" name="nome_proposta" class="form-control" value="<?= $ver->getResult()[0]['nome_proposta'] ?>" />
            </div>
            <div class="form-group">  
            <p class="fonteform" > HTML DE RESPOSTA </p>
            <textarea name="proposta" class="form-control" id="summernote"><?= $ver->getResult()[0]['proposta'] ?> </textarea>
            </div>
            </br>
            <label>
                <input type="hidden" name="id_proposta" value="<?= $ver->getResult()[0]['id_proposta'] ?>" />
                <input type="submit" class="btn btn-primary" name="sendpost" value="cadastrar" />
            </label>
        </form>
    
</div>

