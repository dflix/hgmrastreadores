...
<section>
  ...
  $dadospagto = new Read();
  $dadospagto->ExeRead("baixacaixa","WHERE placa = :p","p={$_GET['id']}");
...
<section> 
    ...
    <?php 
      ...
      $pager = new Pager("index.php?p=dadosacasp&id={$puxa}&atual=", 'Primeira', 'Ultima', '1');
      $pager->ExePager($atual, 10);
      
      $exibeatendimento = new Read();
      $exibeatendimento->ExeRead("atendimentoacasp", "WHERE cliente = :p ORDER BY id DESC LIMIT :limit OFFSET :offset" , "p={$puxa}&limit={$pager->getLimit()}&offset={$pager->getOffset()}");
